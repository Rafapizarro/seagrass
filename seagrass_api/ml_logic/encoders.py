...
def transform_lonlat_features(X: pd.DataFrame, location_features: list) -> pd.DataFrame:
    assert isinstance(X, pd.DataFrame)
    # TODO : set the features with GEOLOCATION
    lonlat_features = [location_features]
...
def compute_geohash(X: pd.DataFrame, precision: int = 5) -> np.ndarray:
    ...
    # TODO : change features for geohash
